...
export async function uploadUsersFromCsvFolder(
  ...
  for (const file of files) {
    ...
    // ensure we always attempt to delete the file after processing (success or failure)
    try {
      let content: string;
      try {
      ...
      // Deduplicate by verification_id (avoid creating same user multiple times)
      ...
      for (let i = 1; i < rawRows.length; i++) {
        ...
        const verification =
          (idxVerification !== -1 ? (values[idxVerification] || '').trim() : (rowData.verification_id || rowData.Verification_no || '')).toString().trim();

        // skip if verification_id is empty or we've already queued it
        if (!verification) continue;
        if (seenVerificationIds.has(verification)) continue;
        seenVerificationIds.add(verification);
            ...
            batch.map(async userData => {
              ...
              try {
                ...
                if (existingUser) {
                  ...
                  // use upsert to reduce race-condition unique failures (if another worker/process creates the same verification_id concurrently)
                  const upserted = await prisma.user.upsert({
                    where: { verification_id: dbData.verification_id },
                    update: dbData,
                    create: dbData
                  });
                  const savedUser = upserted;
      ...
      // capture unexpected per-file errors
      errors.push({ file, message: 'Processing error', error: err });
    } finally {
      // attempt to delete the CSV after processing (successful or not)
      try {
        fs.unlinkSync(filePath);
      } catch (delErr) {
        errors.push({ file, message: 'Unable to delete file after upload', error: delErr });
      }
    }
  } // end files loop
